...
class SetFactory:
    def __init__(self, size: int):
        ...
        self._int_buffer_cache: deque[int] = deque()
        self._ushort_buffer_cache: deque[int] = deque()
        self._bool_buffer_cache: deque[bool] = deque()
        self._float_buffer_cache: deque[float] = deque()
    ...
    def create_bool_set(self, default_state: bool = False, *types: int) -> list[bool]:
        arr = self._get_buffer(self._bool_buffer_cache, bool)
    ...
    def create_int_set(self, default_state: int = -1, *inputs: int) -> list[int]:
        ...
        arr = self._get_buffer(self._int_buffer_cache, int)
        for i in range(self._size):
            arr[i] = default_state
    ...
    def create_float_set(self, default_state: float, *inputs: float) -> list[float]:
        if len(inputs) % 2 != 0:
            raise ValueError("inputs length must be even")
        arr = self._get_buffer(self._float_buffer_cache, float)
        for i in range(self._size):
            arr[i] = default_state
        for j in range(0, len(inputs), 2):
            idx = int(inputs[j])
    ...
    def create_ushort_set(self, default_state: int, *inputs: int) -> list[int]:
        if len(inputs) % 2 != 0:
            raise ValueError("inputs length must be even")
        arr = self._get_buffer(self._ushort_buffer_cache, int)
        for i in range(self._size):
            arr[i] = default_state
        for j in range(0, len(inputs), 2):
            idx = inputs[j]
